...
- [x] Phase 1: Data Layer (types, supabase, improver)
- [x] Phase 2: Floating Selector UI (HTML, CSS, JS)
- [x] Phase 3: Main process integration (keybind, IPC handlers)
- [x] Phase 4: Prompts Management UI (button, view, CRUD)
- [x] Phase 5: Claude-powered Prompt Improvement (Improve Mode)

## Phase 4 Implementation Details
Added "Prompts" button to main window footer that opens a full management view:
- **Files modified:**
  - `/static/index.html` - Added Prompts button and view container
  - `/static/styles/prompts.css` - New styling file (glassmorphism design)
  - `/src/renderer/main.ts` - Added showPrompts(), initPromptsUI(), loadPromptsList(), modal handlers
- **Features:**
  - Category filter tabs (All, Coding, Writing, Analysis, Research, Creative, System)
  - Favorites sorting (starred prompts first)
  - Add/Edit/Delete prompts via modal
  - Click card to use prompt (inserts into home input)
  - Usage tracking (increments count on use)

## Phase 5 Implementation Details (Improve Mode)
Added "Improve Mode" to the floating prompt selector (Cmd+Shift+X) for improving situational prompts with Claude:
- **Files modified:**
  - `/src/main.ts` - Added IPC handler `prompts-improve-with-claude` using Claude API
  - `/src/preload.ts` - Exposed improve functions and streaming listeners
  - `/static/prompt-selector.html` - Added mode toggle tabs (Search/Improve) and improve UI
  - `/static/styles/prompt-selector.css` - Added styling for mode tabs and improve mode
  - `/src/renderer/prompt-selector.ts` - Added mode switching, improve logic, streaming handlers
- **Features:**
  - Mode toggle between Search and Improve modes
  - Textarea for inputting situational/custom prompts
  - "Improve with Claude" button with loading state
  - Streaming response display (shows result as Claude generates)
  - "Use This" button inserts improved prompt into main chat
  - "Copy" button copies improved prompt to clipboard
  - Ctrl/Cmd+Enter shortcut to trigger improvement
- **Technical:**
  - Uses dedicated "Prompt Improver" conversation (is_temporary: true)
  - Conversation persists for history tracking
  - Streams response via IPC events (improve-stream, improve-complete, improve-error)
